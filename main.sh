...
function git_sparse_clone() {  # 保留函数但不再使用，如要可整段删除
  branch="$1" rurl="$2" localdir="$3" && shift 3
  git clone -b $branch --depth 1 --filter=blob:none $rurl $localdir
  cd $localdir
  mv -n $@ ../
  cd ..
  rm -rf $localdir
...
# 原来用 sparse 的 small-package，这里改成完整 clone 再移动需要的目录
...
git clone --depth 1 https://github.com/kenzok8/small-package immpack
cd immpack
